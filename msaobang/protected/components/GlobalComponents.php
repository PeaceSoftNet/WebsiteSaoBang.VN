...
class GlobalComponents {
    ...
    public static function homepageMenu() {
        return array(
            'items' => array(
                array('label' => 'Hướng dẫn', 'url' => array('home/help'), 'itemOptions' => array('class' => 'no-bdl')),
                array('label' => 'Đăng ký', 'url' => array('user/register'), 'visible' => !self::isLogin()),
                array('label' => 'Đăng nhập', 'url' => array('user/login'), 'visible' => !self::isLogin()),
                array('label' => Yii::app()->session['email'], 'url' => array('home/published'), 'visible' => self::isLogin(), 'itemOptions' => array('class' => 'user-name')),
...
//                    $content = str_replace($img, SERVER_MEDIA . Yii::app()->createUrl('external/image', array('code' => base64_encode($img))), $content);
    ...
    public static function processIcon($icon) {
        ...
        if (strpos($icon, "http") !== FALSE) {
            return SERVER_MEDIA . Yii::app()->createUrl('external/image', array('code' => base64_encode($icon)));
        } else {
    ...
    /**
    * Dectect device mobile or computer
    * @author Linhnt
    * @since 10/01/2013
    */
    public static function detectDevice(CEvent $event){
        $urlRedirect = '/m.saobang.vn';
        if( isset($_GET['skin']) && $_GET['skin'] === 'd'){
             Yii::app()->session['theme'] = 0;
        }
        if(!isset(Yii::app()->session['theme']) || Yii::app()->session['theme'] != 0){
            if(Yii::app()->detectMobileBrowser->showMobile){
                Yii::app()->session['theme'] = 1;
                Yii::app()->theme= 'mobile';
                Yii::app()->request->redirect($urlRedirect);
            }
        }
    }
}