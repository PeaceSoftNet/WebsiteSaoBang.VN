...
class TopicModel extends CActiveRecord {
    ...
    public function rules() {
        return array(
            ...
            array('email', 'email'),
            array('mobileNumber', 'length', 'min' => 9, 'max' => 13),
            array('title', 'length', 'max' => 140),
            array('verifyCode', 'captcha', 'allowEmpty' => !CCaptcha::checkRequirements(), 'message' => 'Mã xác thực không chính xác'),
//            array('hash1', 'unique', 'message' => 'Bạn đã đăng bài có tiêu đề tương tự'),
//            array('hash2', 'unique', 'message' => 'Bạn đã đăng bài có nội dung tương tự'),
        );
    }
    ...
    public function afterSave() {
        $topicModel = Yii::app()->cache->delete('TopicModel_' . $this->id);
        $topicDetail = Yii::app()->cache->delete('TopicDetail_' . $this->id);
        //update to solr
        $listFillter = array(
            'id' => $this->id,
            'title' => $this->title,
        ...
        $data = Yii::app()->CURL->run('http://' . $_SERVER["HTTP_HOST"] . '/topic/AppendSolr', FALSE, $listFillter);
