 ...
 * <pre>
 * $document = ASolrAskDocument::model()->findByPk(1); // loads a document with a unique id of 1
 ...
 * @package application.components.solr
 */
class ASolrAskDocument extends CFormModel implements IASolrDocument {

    /**
     * The solr connection
     * @var ASolrAskDocument
     */
    public static $solr;
    ...
    public function attributeNames() {
        return CMap::mergeArray(
                        array_keys($this->_attributes->toArray()), parent::attributeNames()
        );
    }
     ...
     * @return ASolrAskDocument solr document model instance.
     */
    public static function model($className = __CLASS__) {
     ...
     * @return ASolrAskDocument the solr connection used by solr document.
     */
    public function getSolrConnection() {
        if ($this->_connection !== null) {
            ...
            self::$solr = Yii::app()->solrAsk;
            if (self::$solr instanceof IASolrConnection)
                return self::$solr;
            else
                throw new CException(Yii::t('yii', 'Solr Document requires a "solr" ASolrAskDocument application component.'));
        }
    }

    /**
     * Sets the solr connection used by this solr document
     * @param ASolrAskDocument $connection the solr connection to use for this document
     */
    public function setSolrConnection(ASolrAskDocument $connection) {
     ...
     * @return ASolrAskDocument the active record
     */
     ...
     * @return ASolrAskDocument the newly created solr document. The class of the object is the same as the model class.
...
//		Yii::trace(get_class($this).'.count()','application.components.solr.ASolrAskDocument');
        if ($criteria === null) {
     ...
     * @param ASolrCriteria $criteria solr query criteria.
     * @return ASolrAskDocument the document found. Null if none is found.
     */
    public function find(ASolrCriteria $criteria = null) {
//		Yii::trace(get_class($this).'.find()','application.components.solr.ASolrAskDocument');
        if ($criteria === null) {
            $criteria = new ASolrCriteria();
        }
     ...
     * @param ASolrCriteria $criteria solr query criteria.
     * @return ASolrAskDocument[] the documents found.
     */
    public function findAll(ASolrCriteria $criteria = null) {
//		Yii::trace(get_class($this).'.findAll()','application.components.solr.ASolrAskDocument');
        if ($criteria === null) {
            $criteria = new ASolrCriteria();
        }
...
//		Yii::trace(get_class($this).'.findByPk()','application.components.solr.ASolrAskDocument');
        if ($criteria === null) {
            $criteria = new ASolrCriteria();
        }
        if (is_array($pk)) {
...
//		Yii::trace(get_class($this).'.findByAttributes()','application.components.solr.ASolrAskDocument');
        if ($criteria === null) {
            $criteria = new ASolrCriteria();
        }
        $query = array();
...
//		Yii::trace(get_class($this).'.findAllByAttributes()','application.components.solr.ASolrAskDocument');
        if ($criteria === null) {
            $criteria = new ASolrCriteria();
        }
        $query = array();
