 ...
 */
class ASolrAskConnection extends CApplicationComponent implements IASolrConnection {
    ...
    public function index($document, $commitWithin = null) {
        if ($document instanceof IASolrDocument) {
            ...
            Yii::trace('Adding ' . count($document) . " documents to the solr index", 'application.components.solr.ASolrAskConnection');
            return $this->getClient()->addDocuments($document, false, $commitWithin)->success();
        }
        if ($commitWithin === null) {
            $commitWithin = 0;
        }
        Yii::trace('Adding 1 document to the solr index', 'application.components.solr.ASolrAskConnection');
    ...
    public function delete($document) {
        if ($document instanceof IASolrDocument) {
            ...
            Yii::trace('Deleting From Solr IDs: ' . implode(", ", $document), 'application.components.solr.ASolrAskConnection');
            return $this->getClient()->deleteByIds($document)->success();
        }
        Yii::trace('Deleting From Solr ID: ' . $document, 'application.components.solr.ASolrAskConnection');
    ...
    public function search(ASolrCriteria $criteria, $modelClass = "ASolrDocument") {
        ...
        Yii::trace('Querying Solr: ' . ((string) $c), 'application.components.solr.ASolrAskConnection');
    ...
    public function count(ASolrCriteria $criteria) {
        ...
        $c->mergeWith($criteria);
        $c->setLimit(0);
        Yii::trace('Counting Results from Solr: ' . ((string) $c), 'application.components.solr.ASolrAskConnection');
    ...
    protected function rawSearch(ASolrCriteria $criteria) {
        //thiendd@peacesoft.net turn off log
//            if ($this->enableProfiling) {
//                    $profileTag = "application.components.solr.AConnection.rawSearch(".$criteria->__toString().")";
